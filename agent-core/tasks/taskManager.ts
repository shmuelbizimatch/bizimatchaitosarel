...
import { Task, TaskStatus, AgentType, ExecutionMode, TaskMetadata, AIEngine } from '../../types';
...
export class TaskManager {
  ...
  async createTask(
    ...
    const task: Task = {
      ...
      metadata: {
        ...
        retry_count: 0,
        ai_engine: metadata?.ai_engine || (process.env.DEFAULT_AI_ENGINE as AIEngine) || 'claude',
  ...
  async failTask(taskId: string, errorMessage: string, errorStack?: string): Promise<void> {
    try {
      const now = new Date().toISOString();
      const task = this.activeTasks.get(taskId);
  ...
  private async retryTask(originalTask: Task): Promise<void> {
    try {
      ...
      const retryTask = await this.createTask(
        ...
        originalTask.agent_type,
        originalTask.input_data,
  ...
  async createWorkflow(
    ...
    const orchestratorTask = await this.createTask(
      projectId,
      mode,
      'orchestrator',
      inputData,
  ...
  async cleanupCompletedTasks(retentionDays: number = 7): Promise<void> {
    try {
  ...
  // Get task statistics
  async getTaskStatistics(projectId?: string): Promise<Record<string, any>> {
    try {
      let query = this.supabase
        .from('task_summary')
        .select('*');
