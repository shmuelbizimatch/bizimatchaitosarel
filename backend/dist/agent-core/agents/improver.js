...
class ImproverAgent {
    ...
    async gatherProjectInsights(projectId) {
        try {
    ...
    async analyzeComponentsForUX(scanResults, projectPath) {
        ...
        try {
            ...
            const componentFiles = scanResults.file_details?.filter((file) => file.type === 'component' ||
            ...
            for (const fileDetail of componentFiles.slice(0, 10)) { // Limit to 10 files for performance
                try {
                    const filePath = path_1.default.join(projectPath, fileDetail.path);
    ...
    analyzeComponentFile(content, filePath) {
        ...
        if (content.includes('useState'))
            analysis.ux_patterns.push('State Management');
        if (content.includes('useEffect'))
            analysis.ux_patterns.push('Side Effects');
        if (content.includes('form') || content.includes('Form'))
            analysis.ux_patterns.push('Forms');
        if (content.includes('loading') || content.includes('Loading'))
            analysis.ux_patterns.push('Loading States');
        if (content.includes('error') || content.includes('Error'))
            analysis.ux_patterns.push('Error Handling');
        if (content.includes('modal') || content.includes('Modal'))
            analysis.ux_patterns.push('Modal Dialogs');
        if (content.includes('button') || content.includes('Button'))
            analysis.ux_patterns.push('Interactive Buttons');
        if (content.includes('nav') || content.includes('Nav'))
            analysis.ux_patterns.push('Navigation');
        return analysis;
    }
    async generateAIEnhancements(scanResults, componentAnalysis, projectInsights, targetComponent) {
        try {
            ...
            const aiRequest = AIClient_1.AIClient.getImproverPrompt(enhancedScanResults, targetComponent);
    ...
    async performLocalUXAnalysis(componentAnalysis, projectPath) {
        const localEnhancements = [];
        try {
            ...
            if (componentAnalysis.accessibility_issues.length > 0) {
                localEnhancements.push({
                    ...
                    code_changes: componentAnalysis.accessibility_issues.map((issue) => ({
            ...
            if (componentAnalysis.performance_issues.length > 0) {
                localEnhancements.push({
                    ...
                    code_changes: componentAnalysis.performance_issues.map((issue) => ({
                        file_path: issue.file_path,
                        change_type: 'modify',
    ...
    generateAccessibilityFix(issue) {
        switch (true) {
...
export const theme = {
  ...
  typography: {
    ...
    createFallbackEnhancements(scanResults, componentAnalysis) {
    ...
    mergeEnhancements(aiEnhancements, localEnhancements) {
        const merged = [...aiEnhancements, ...localEnhancements];
        // Remove duplicates based on description similarity
        const unique = merged.filter((enhancement, index, self) => index === self.findIndex(e => e.description.toLowerCase().includes(enhancement.description.toLowerCase().substring(0, 20))));
    ...
    createImplementationPlan(enhancements) {
        const steps = [];
    ...
    extractDependencies(enhancement) {
        const dependencies = [];
    ...
    calculateUXScore(scanResults, componentAnalysis, phase, enhancements) {
        ...
        if (phase === 'after' && enhancements) {
            const totalUXImprovement = enhancements.reduce((sum, enhancement) => sum + (enhancement.impact_assessment.user_experience / 10), 0);
            score += totalUXImprovement;
        }
        // Clamp between 1-10
        return Math.max(1, Math.min(10, score));
    }
    async storeImprovementInsights(projectId, enhancements, uxScoreBefore, uxScoreAfter) {
        try {
            // Store improvement insights
            await this.memoryManager.storeInsight(projectId, `UX improvement analysis completed with ${enhancements.length} enhancements`, {
            ...
            for (const enhancement of highImpactEnhancements) {
                await this.memoryManager.storePattern(projectId, `High-impact enhancement: ${enhancement.enhancement_type}`, [enhancement.description], 1, 6);
            }
...
exports.ImproverAgent = ImproverAgent;
